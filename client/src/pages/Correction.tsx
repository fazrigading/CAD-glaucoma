...
// Polygon & Point interface (bisa dipindah ke file interface jika ingin)
interface Point {
  x: number;
  y: number;
}
...
// Fungsi untuk menghitung h_cdr berdasarkan polygon disc dan cup
const calculateHCdr = (discPolygons: Polygon[], cupPolygons: Polygon[]): number | null => {
  // Validasi: pastikan ada polygon disc dan cup dengan minimal 3 titik
  ...
  // Ambil semua titik dari polygon disc dan cup
  ...
  // Cari titik terkiri dan terkanan untuk disc
  ...
  // Cari titik terkiri dan terkanan untuk cup
  ...
  // Hitung h_cdr sebagai rasio jarak horizontal cup terhadap disc
  if (discHorizontalDistance === 0) {
    return null;
  }

  const hCdr = cupHorizontalDistance / discHorizontalDistance;
  return Math.round(hCdr * 100) / 100; // Bulatkan ke 2 desimal
};

// Fungsi untuk menghitung v_cdr berdasarkan polygon disc dan cup
const calculateVCdr = (discPolygons: Polygon[], cupPolygons: Polygon[]): number | null => {
  if (discPolygons.length === 0 || cupPolygons.length === 0) {
    return null;
  }
...
// Fungsi untuk menghitung luas polygon menggunakan Shoelace formula
...
// Fungsi untuk menghitung area_cdr berdasarkan polygon disc dan cup
const calculateAreaCdr = (discPolygons: Polygon[], cupPolygons: Polygon[]): number | null => {
  // Validasi: pastikan ada polygon disc dan cup dengan minimal 3 titik
  if (discPolygons.length === 0 || cupPolygons.length === 0) {
    return null;
  }
  ...
  // Hitung total luas disc
  let totalDiscArea = 0;
  validDiscPolygons.forEach(polygon => {
    totalDiscArea += calculatePolygonArea(polygon.points);
  });

  // Hitung total luas cup
  let totalCupArea = 0;
  validCupPolygons.forEach(polygon => {
    totalCupArea += calculatePolygonArea(polygon.points);
  });

  // Hitung area_cdr sebagai rasio luas cup terhadap disc
  if (totalDiscArea === 0) {
    return null;
  }

  const areaCdr = totalCupArea / totalDiscArea;
  return Math.round(areaCdr * 100) / 100; // Bulatkan ke 2 desimal
};
...
function Correction() {
  ...
  const fetchPolygonData = async (patientId: number) => {
    try {
      ...
      if (result.success && result.data) {
          ...
          polygonData.disc_polygons.forEach((discPolygon: any, index: number) => {
            if (discPolygon && Array.isArray(discPolygon.points) && discPolygon.points.length > 0) {
              ...
              loadedPolygons.push({
                ...
                label: 'disc',
                points: discPolygon.map((point: any) => ({
                  x: Number(point.x) || 0,
                  y: Number(point.y) || 0
                }))
              });
          ...
          polygonData.cup_polygons.forEach((cupPolygon: any, index: number) => {
            if (cupPolygon && Array.isArray(cupPolygon.points) && cupPolygon.points.length > 0) {
              ...
              loadedPolygons.push({
                ...
                label: 'cup',
                points: cupPolygon.map((point: any) => ({
                  x: Number(point.x) || 0,
                  y: Number(point.y) || 0
                }))
              });
        ...
        console.log('No polygon data found for patient:', patientId);
        setPolygons([]);
      }
    } catch (error) {
  ...
  // Dummy data for CorrectionResult preview
  ...
  const handleDownloadBackground = async () => {
    ...
    try {
      ...
      if ('showSaveFilePicker' in window) {
        // @ts-ignore - File System Access API belum fully supported di TypeScript
